...
- `Outliner & Fold`：为主编辑器与 `Peek Drawer` 提供 TOC、大纲导航与折叠能力
  - 当前第一阶段已落地主编辑器与 `Peek Drawer`：`/write` 左侧低存在感 Outliner 与抽屉内文索引都会实时抽取 `H1 / H2 / H3`，并提供平滑滚动跳转、`Scroll Spy`、空状态与长标题截断
  - 标题折叠已进入轻量原型态：主编辑器当前通过 `TipTap / ProseMirror DecorationSet` 隐藏正文区段，用于验证交互价值；保存用内容结构保持不变，且仍未把重型自定义节点接入主链
- 明确放弃在纯 Web 架构中以 `IndexedDB` 解决富媒体容量瓶颈的路线；媒体承载已转入桌面原生阶段处理

当前架构判断：`v1.2` 已完成当前阶段收口。
本轮结案依据为代码主链、文档同步与 `npm run lint` / `npm run build` 的通过结果；人工体感验收延后到下一阶段回归，不再阻塞本次里程碑关闭。
