...
- 真实样本级推荐回归，而不只依赖离线靶场与单点脚本
- 推荐阈值、排序策略与命中解释的收口
- `vectorCache` 的生命周期与大库下重建策略
- `entities / semantic / both` 三类命中的 UI 文案和提示强度统一
- 明确且文档化的失败回退边界

判断 `v1.1` 是否成立时，默认以 [`04-CHANGELOG.md`](./04-CHANGELOG.md) 中新增的“`v1.1` 结案标准”段落为准。
...
3. 先确认本轮任务属于 `v1.1`、`v1.2`、`v1.3`、`v2.0-Alpha`、`v2.0`、`v2.1`、`v2.2` 或 `v2.3` 中的哪一个。
