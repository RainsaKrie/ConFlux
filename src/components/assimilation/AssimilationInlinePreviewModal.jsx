...
import { buildAssimilationInlineDiffHtml } from '../../features/assimilation/diff'
import { useTranslation } from '../../i18n/I18nProvider'

const MotionDiv = motion.div

export function AssimilationInlinePreviewModal({ preview, onApply, onCancel }) {
  const { t } = useTranslation()

  const inlineDiffHtml = useMemo(
    () => buildAssimilationInlineDiffHtml(preview?.beforeContent ?? '', preview?.afterContent ?? ''),
    [preview?.afterContent, preview?.beforeContent],
  )
